...
class GUI:
    ...
    def create_button_frame(self, root):
        ...
        akko_button = tk.Button(button_frame, text="Process Akko".upper(),
                                command=lambda: self.on_button_click("akko_process"), **button_style())
        akko_button.grid(row=0, column=0, padx=5)

        cps_button = tk.Button(button_frame, text="Process CellphoneS".upper(),
                               command=lambda: self.on_button_click("cps_process"), **button_style())
