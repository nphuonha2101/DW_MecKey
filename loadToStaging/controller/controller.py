...
class Controller:
    ...
    def begin_process(self, event_data: dict):
        process = injector.get(Processing)
        if event_data["process_name"] == "akko_process":
            ...
            if status is None or file_path is None:
                process.set_feed_key("akko_feed")
                process.run()
                return
            process.run(status, file_path)

        elif event_data["process_name"] == "cps_process":

            status = event_data.get("status", None)
            file_path = event_data.get("file_path", None)

            if status is None or file_path is None:
                process.set_feed_key("cps_feed")
                process.run()
                return
