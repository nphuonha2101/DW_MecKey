...
class GUI:
    ...
    def create_button_frame(self, root):
        ...
        akko_button = tk.Button(button_frame, text="Load Akko".upper(),
                                command=lambda: self.on_button_click("akko_load"), **button_style())
        akko_button.grid(row=0, column=0, padx=5)

        cps_button = tk.Button(button_frame, text="Load CellphoneS".upper(),
                               command=lambda: self.on_button_click("cps_load"), **button_style())
