...
class Controller:
    ...
    def begin_process(self, event_data: dict):
        if event_data["process_name"] == "akko_extract":
            ...
            if status is None or file_path is None:
                akko_extract.run()
                return
            akko_extract.run(status, file_path)

        elif event_data["process_name"] == "cps_extract":
            cps_extract = injector.get(CellphoneSExtract)
            ...
            cps_extract.run(status, file_path)
