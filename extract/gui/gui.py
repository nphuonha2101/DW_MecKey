...
class GUI:
    ...
    def create_button_frame(self, root):
        ...
        akko_button = tk.Button(button_frame, text="Extract Akko".upper(),
                                command=lambda: self.on_button_click("akko_extract"), **button_style())
        akko_button.grid(row=0, column=0, padx=5)

        cps_button = tk.Button(button_frame, text="Extract CellphoneS".upper(),
                               command=lambda: self.on_button_click("cps_extract"), **button_style())
